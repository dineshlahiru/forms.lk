...
import { getCustomForms, getPhoneNumbers, type StoredForm } from '../utils/formsStorage';
...
export function FormsLibraryPage() {
  ...
  }, [allForms, searchQuery, selectedCategory, sortBy]);
    ...
    <Layout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {filteredForms.length > 0 ? (
          <>
            {viewMode === 'list' ? (
              <ListView forms={paginatedForms} />
            ) : (
              <GridView forms={paginatedForms} />
            )}
...
function ListView({ forms }: { forms: Form[] }) {
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <div className="divide-y divide-gray-100">
        {forms.map((form) => (
          <ListViewRow key={form.id} form={form} />
        ))}
      </div>
    </div>
  );
...
function ListViewRow({ form }: { form: Form }) {
...
function GridView({ forms }: { forms: Form[] }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {forms.map((form) => (
        <GridViewCard key={form.id} form={form} />
      ))}
    </div>
  );
}

function GridViewCard({ form }: { form: Form }) {
  const verification = VERIFICATION_LEVELS[form.verificationLevel];
  const phones = form.telephoneNumbers?.slice(0, 2) || [];
  const website = form.website;
  const email = form.email;
