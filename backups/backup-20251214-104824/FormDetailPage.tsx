...
import { getFormById as getCustomFormById, updateForm as updateStoredForm, getPhoneNumbers, createContactInfo, getPdfPagesForLanguage, getFormTitle, type StoredForm } from '../utils/formsStorage';
...
export function FormDetailPage() {
  ...
  // Get stored form for language variants
  const storedForm = useMemo(() => getCustomFormById(formId || ''), [formId]);

  // Language state
  const [selectedLanguage, setSelectedLanguage] = useState<Language>('en');
  // Check both languages array and languageVariants for available languages
  const availableLanguages = useMemo(() => {
    if (!storedForm) return ['en'];
    // First check languageVariants (preferred source)
    if (storedForm.languageVariants && storedForm.languageVariants.length > 0) {
      return storedForm.languageVariants.map(v => v.language);
    }
    // Fall back to languages array
    if (storedForm.languages && storedForm.languages.length > 0) {
      return storedForm.languages;
    }
    return ['en'];
  }, [storedForm]);
  ...
  // Get PDF pages for selected language
  const currentPdfPages = useMemo((): string[] | undefined => {
    if (!storedForm) return undefined;
    return getPdfPagesForLanguage(storedForm, selectedLanguage);
  }, [storedForm, selectedLanguage]);
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            ...
            <div className="flex-1">
              {isTitleEditing ? (
                ...
                >
                  {storedForm ? getFormTitle(storedForm, selectedLanguage) : form.title}
                </h1>
              )}
              <div className="flex items-center justify-between">
                <p className="text-[#718096]">{form.institution}</p>
                {/* Language Selector - inside form name block, right bottom */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex gap-1">
                      {availableLanguages.map((lang) => (
                        ...
                        >
                          {LANGUAGES[lang as Language]?.nativeLabel || lang}
                        </button>
                      ))}
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
              ...
              {showPreview && (
                <div className="border-t border-gray-200">
                  {currentPdfPages && currentPdfPages.length > 0 ? (
                    <div className="p-4 bg-gray-50 space-y-4 max-h-[600px] overflow-y-auto">
                      {currentPdfPages.map((pageImg: string, i: number) => (
